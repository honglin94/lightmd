...
void handleHttp(const HttpRequest& request, HttpResponse& response)
{
    std::cout << "request: " << request.getMethod() << " " << request.getPath() << std::endl;
    const std::map<string, string>& headers = request.getHeaders();
    ...
    if(request.getPath() == "/")
    {
        ...
        response.addHeader("Server", "lightmd");
        response.setBody("<html><head><title>This is titile</title></head>"
            "<body><h1>Hello</h1>Now is" + Timestamp::now().toString() + "</body></html>");
    }
...
int main(int argc, char* argv[])
{
    ...
    serverAddr.sin_port = htons(12019);
    HttpServer server(&loop, &serverAddr);
    server.setHttpCallback(handleHttp);
    server.start();
    loop.loop();
}
